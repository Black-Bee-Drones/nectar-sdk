"""Shared multicopter vehicle behavior, driven by a pluggable transport."""
...
class VehicleDrone(BaseDrone):
    ...
    def __init__(
        ...
        self._navigator = VehicleNavigator(self)

        self._startup_sensors()
        self._load_pid_config()
        self._load_firmware_config()
    ...
    def _command_land(self) -> bool:
        """Issue the firmware land command at the current position."""
        return self._transport.command_land()

    def _ensure_offboard_ready(self) -> None:
        """Ensure the FCU is ready to accept offboard setpoints.

        No-op by default (ArduPilot GUIDED persists). PX4 overrides this to
        (re)enter OFFBOARD mode and keep its setpoint stream alive.
        """

    def _prepare_position_setpoint(self, precision: float) -> None:
        """Hook run before a POSITION/POSITION_GLOBAL setpoint is published.

        No-op by default; ArduPilot uses it to sync ``WPNAV_RADIUS``.
        """

    def _emit_velocity(
        self, vx: float, vy: float, vz: float, yaw_rate: float, frame: TargetFrame
    ) -> None:
        """Send a velocity setpoint to the transport.

        Routed through the drone so firmware layers (PX4) can capture the last
        setpoint for continuous offboard streaming.
        """
    ...
    def rtl(
        ...
        method : RTLMethod, default=NAVIGATE
            ...
            NATIVE: trigger the autopilot's native RTL mode.
        ...
        if method == RTLMethod.NATIVE:
            return self._rtl_native(altitude, land)
