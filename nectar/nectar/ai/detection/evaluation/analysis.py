"""PR curve analysis and error statistics for object detection evaluation."""
...
def process_evaluation_results(
    ...
    for class_id in range(num_classes):
        ...
        for thresh in thresholds:
            p, r, f1 = _metrics_at_threshold(scores, true_labels, thresh, total_gt)
            precisions.append(p)
            recalls.append(r)
            f1_scores.append(f1)
...
def create_error_statistics(
    ...
    false_positives: List[Tuple[int, float, int]] = []
...
def save_error_statistics_csv(
    ...
    df.to_csv(eval_dir / "error_statistics.csv", index=False)
