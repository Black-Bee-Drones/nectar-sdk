...
class OakdCam(DepthCam):
    ...
    def _require_depthai() -> None:
        if dai is None:
            raise RuntimeError(
                "DepthAI (depthai) is not installed. Install it to use OakdCam: pip install depthai"
            )
    ...
    def create_yolo_detection_network(
        ...
        confidence = (
            confidence if confidence < 1.0 and confidence > 0.0 else self.confidence
        )
    ...
    def __get_model_settings(self, json_path: Path | str) -> None:
        ...
        if self.nn_config is None:
            raise ValueError("nn_config property not found in json file")
        self.metadata: dict = self.nn_config.get("NN_specific_metadata", None)
        self.labels: list = json_file.get("mappings", None).get("labels", None)
