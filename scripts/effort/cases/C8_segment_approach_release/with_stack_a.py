...
classifier.load()
handler = ImageHandler("oakd")
handler.open()


def get_range(cx: float, cy: float, pixel_h: float):
    del pixel_h  # OAK-D depth path; height unused
    if handler.camera is None:
        return None
    d = handler.camera.get_distance(int(cx), int(cy))
    return float(d) if d is not None else None


def hold_payload() -> None:
    drone.do_servo(1, 1000)


def release_payload() -> None:
    drone.do_servo(1, 2000)
