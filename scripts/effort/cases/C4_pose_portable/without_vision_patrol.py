...
#!/usr/bin/env python3
"""VISION patrol without Nectar (MAVROS position + vision_pose relay)."""
...
class VisionPatrol(Node):
    def __init__(
        self,
        slam_topic: str = "/visual_slam/tracking/vo_pose_covariance",
        vision_pose_topic: str = "/mavros/vision_pose/pose_cov",
    ) -> None:
        super().__init__("vision_patrol")
        qos = QoSProfile(
        ...
        # PoseSource.VISION wiring (absent on the GPS Without path).
        self._vision_pub = self.create_publisher(
            PoseWithCovarianceStamped, vision_pose_topic, qos_profile_sensor_data
        )
        self.create_subscription(
    ...
    def _on_vision_pose(self, msg: PoseWithCovarianceStamped) -> None:
        self._vision_pub.publish(msg)
